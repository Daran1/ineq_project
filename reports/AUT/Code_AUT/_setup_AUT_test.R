...
silc.rph <- silc.rph %>%
  mutate(p11 = py010g + py050g + py080g + car)
...
silc.rph <- silc.rph %>% 
  mutate(Can_inc = (sp11 + hy110g + hy040g + hy090g) / hx050)

#Income 2: Pre-tax national income

#sum up for every person
silc.rph <- silc.rph %>% 
  mutate(p12 = py090g + py100g)

#sum up for household
silc.rph <- silc.rph %>% group_by(id_h) %>%
  mutate(sp12 = sum(p12))

# equivalise, allocate p.p.
silc.rph <- silc.rph %>%
  mutate(prenatincom = Can_inc + (sp12 / hx050))
...
# Post tax income 
silc.rph <- silc.rph %>%
  mutate(posttax = (prenatincom + (sp13 + hy050g + hy060g + hy070g + hy080g 
...
#silc.rph <- silc.rph %>%
#  mutate(income_wid_1 = py010g + py050g + py080g + car +
#           (hy110g + hy040g + hy090g) / n)

silc.rph <- silc.rph %>%
  mutate(income_wid_1 = sp11 + 
           (hy110g + hy040g + hy090g) / n)
# Pre-tax income
